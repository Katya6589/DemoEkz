...
from pages.db import get_partners
from pages.AddPartner import AddPartner
from pages.EditPartner import EditPartner
...
class PartnersView:
    def __init__(self, ui, cursor):
        ...
        self.VivodParynera = self.ui.findChild(QListWidget, "VivodParynera")
        self.btn_add = self.ui.findChild(QPushButton, "btnAddPartner")

        self.btn_add.clicked.connect(self.add_partner)
        self.VivodParynera.itemClicked.connect(self.show_partner_details)
        self.ui.stackedWidget.currentChanged.connect(self.on_page_changed) # не менять
